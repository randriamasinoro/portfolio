...
demo: https://indian-food.duckdns.org
featured: true
---
...
| DNS | DuckDNS | DNS dynamique gratuit pour IP variable |
| SSL | Let's Encrypt + Certbot | Certificat automatisé, renouvellement par cron |
| Reverse proxy | Nginx (host) | Terminaison TLS, séparation des responsabilités |
...
Navigateur
    ...
    ▼
DuckDNS → IP Oracle Cloud
    │ port 443
    ▼
Nginx host  ← TLS terminé ici (Let's Encrypt)
    │ http://localhost:8001
    ▼
...
- **DuckDNS** : solution de DNS dynamique gratuite, fiabilité non garantie pour un usage professionnel.
